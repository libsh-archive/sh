    ...
    SH_WHILE(mid - start > tframe.eps && end - mid > tframe.eps) { 
      ...
      if(use_aa || debugMode == TRACE) {
        ...
        if(use_aa) {
          ...
          Attrib1f SH_DECL(otherError) = resultRadius - abs(inError);
      ...
      if(debugMode == CUTOFF || debugMode == PLOT) {
        ...
        /* range graph on top */
        v = ((v - graphStart2) / graphHeight - 0.5) * tframe.graphScale;
        Attrib1f inRangeAA = v > range_lo(resultRangeAA) && v < range_hi(resultRangeAA); 
        Attrib1f inRangeIA = v > range_lo(resultRangeIA) && v < range_hi(resultRangeIA); 
        Attrib1f inRangeBoth = inRangeAA && inRangeIA; 
        Attrib1f inZero = abs(v) < tframe.graphScale / 200.0;
        ...
        Color3f dbgColor = lerp(inDomainGraph, domainColor, 
                           lerp(inGap, bkgndColor, rangeColor));
