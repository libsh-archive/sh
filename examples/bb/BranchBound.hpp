 ...
 * Right now only handles SH::ShPrograms with single value output
...
template<typename T> 
struct IAStream: public SH::ShStream {
  typedef typename SH::ShStorageTypeInfo<typename T::storage_type>::IntervalType IntervalStorageType; 
  typedef SH::ShAttrib<T::typesize, T::binding_type, IntervalStorageType> IntervalT;
  ...
  /* Appends an element, automatically doubling reserved capacity
   * if necessary. */
  ...
  IAStream& operator=(const SH::ShProgram& program);
  ...
  private:
    ...
    SH::ShChannel<T> m_lo, m_hi;

    // never null, always same size as capacity() * m_datasize
    typedef typename T::VariantType DataType;
    typedef SH::ShPointer<DataType> DataTypePtr; 
    DataTypePtr m_loData, m_hiData; 

    void resize(int length, SH::ShChannel<T>& ch, DataTypePtr& data);
    void reserve(int length, SH::ShChannel<T>& ch, DataTypePtr& data);
    ...
    static void allocData(int length, SH::ShChannel<T>& ch, DataTypePtr& data);
};
...
class BranchBound {
  public:
    ...
    BranchBound(SH::ShProgram f, size_t splitLevel=1, HostType eps = 1e-7);
