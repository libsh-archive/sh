    ...
    m_schedule(schedule)
{
  ...
  GET_WGL_PROCEDURE(glBeginOcclusionQueryNV, GLBEGINOCCLUSIONQUERYNV);
  GET_WGL_PROCEDURE(glGenOcclusionQueriesNV, GLGENOCCLUSIONQUERIESNV);
  GET_WGL_PROCEDURE(glDeleteOcclusionQueriesNV, GLDELETEOCCLUSIONQUERIESNV);
  GET_WGL_PROCEDURE(glEndOcclusionQueryNV, GLENDOCCLUSIONQUERYNV);
  GET_WGL_PROCEDURE(glGetOcclusionQueryuivNV, GLGETOCCLUSIONQUERYUIVNV);

#ifdef EXT_framebuffer_object
 GET_WGL_PROCEDURE(glIsRenderbufferEXT, GLISRENDERBUFFEREXTPROC);
 GET_WGL_PROCEDURE(glBindRenderbufferEXT, GLBINDRENDERBUFFEREXTPROC);
 GET_WGL_PROCEDURE(glDeleteRenderbuffersEXT, GLDELETERENDERBUFFERSEXTPROC);
 GET_WGL_PROCEDURE(glGenRenderbuffersEXT, GLGENRENDERBUFFERSEXTPROC);
 GET_WGL_PROCEDURE(glRenderbufferStorageEXT, GLRENDERBUFFERSTORAGEEXTPROC);
 GET_WGL_PROCEDURE(glGetRenderbufferParameterivEXT, GLGETRENDERBUFFERPARAMETERIVEXTPROC);
 GET_WGL_PROCEDURE(glIsFramebufferEXT, GLISFRAMEBUFFEREXTPROC);
 GET_WGL_PROCEDURE(glBindFramebufferEXT, GLBINDFRAMEBUFFEREXTPROC);
 GET_WGL_PROCEDURE(glDeleteFramebuffersEXT, GLDELETEFRAMEBUFFERSEXTPROC);
 GET_WGL_PROCEDURE(glGenFramebuffersEXT, GLGENFRAMEBUFFERSEXTPROC);
 GET_WGL_PROCEDURE(glCheckFramebufferStatusEXT, GLCHECKFRAMEBUFFERSTATUSEXTPROC);
 GET_WGL_PROCEDURE(glFramebufferTexture1DEXT, GLFRAMEBUFFERTEXTURE1DEXTPROC);
 GET_WGL_PROCEDURE(glFramebufferTexture2DEXT, GLFRAMEBUFFERTEXTURE2DEXTPROC);
 GET_WGL_PROCEDURE(glFramebufferTexture3DEXT, GLFRAMEBUFFERTEXTURE3DEXTPROC);
 GET_WGL_PROCEDURE(glFramebufferRenderbufferEXT, GLFRAMEBUFFERRENDERBUFFEREXTPROC);
 GET_WGL_PROCEDURE(glGetFramebufferAttachmentParameterivEXT, 
		   GLGETFRAMEBUFFERATTACHMENTPARAMETERIVEXTPROC);
 GET_WGL_PROCEDURE(glGenerateMipmapEXT, GLGENERATEMIPMAPEXTPROC);
#endif
