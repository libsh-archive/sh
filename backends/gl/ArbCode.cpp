...
void dump(const ProgramNodePtr& foo, string desc) {
  ...
  Structural st(foo->ctrlGraph);
  std::ostringstream structOut; 
  st.dump(structOut);
  dotGen(structOut.str(), desc + "_struct");

  SectionTree sec(st); 
  std::ostringstream secOut;
  sec.dump(secOut);
  dotGen(secOut.str(), desc + "_sec");
#endif
}
...
void ArbCode::generate()
{
  ...
  try {
    if (m_environment & ARB_NVFP2) {
      ...
      m_shader->ctrlGraph->clear_marked();
  ...
  if (I == m_registers.end()) {
    if ((1 == var.size()) && (SH_CONST == var.node()->kind())) {
      ...
      cerr << "No register allocated for variable '" << var.name() << "' (size=" 
		<< var.size() << "; kind=" << var.node()->kind() << "; object=" << var.node().object() << ")" << endl;
