...
class ArbStreamException : public ShException {
public:
  ArbStreamException(const std::string& message)
    : ShException("ARB Backend Stream Execution: " + message)
  {
  }
};
                         ...
                         ShStream& dest)
{
  ...
  if (!program->inputs.empty()) {
    ShError(ArbStreamException("Stream program has unbound inputs, and can hence not be executed."));
    return;
  }
  ...
  if (input_map.empty()) {
    ShError(ArbStreamException("Stream program does not use any streams!"));
    return;
  }
  ...
  if (!pbuffer) {
    ShError(ArbStreamException("Could not make pbuffer!"));
    XCloseDisplay(dpy);
    return;
  }
  ...
  if (!pbuffer_ctxt) {
    ShError(ArbStreamException("Could not create PBuffer context"));
    XFree(fb_config);
    XCloseDisplay(dpy);
    return;
  }
  ...
  if (gl_error != GL_NO_ERROR) {
    ShError(ArbStreamException("Could not enable GL_VERTEX_PROGRAM_ARB"));
  ...
  gl_error = glGetError();
  if (gl_error != GL_NO_ERROR) {
    ShError(ArbStreamException("Could not enable GL_FRAGMENT_PROGRAM_ARB"));
    glXDestroyContext(dpy, pbuffer_ctxt);
    XFree(fb_config);
    XCloseDisplay(dpy);
    return;
  ...
  gl_error = glGetError();
  if (gl_error != GL_NO_ERROR) {
    ShError(ArbStreamException("Could not do glReadPixels()"));
    glXDestroyContext(dpy, pbuffer_ctxt);
    XFree(fb_config);
    XCloseDisplay(dpy);
    return;
