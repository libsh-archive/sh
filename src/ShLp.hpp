...
struct SH_DLLEXPORT ShInfeasibleLpException {};

/* Exception thrown when a linear program is unbounded */
struct SH_DLLEXPORT ShUnboundedLpException {};
...
std::valarray<T> shLpSimplex(const std::valarray<T>& A, 
 ...
 */
template<int M, int N, typename T>
std::valarray<T> shLpSimplexRelaxed(const std::valarray<T>& A, 
    const std::valarray<T>& b, const std::valarray<T>& c,
    const std::valarray<bool>& bounded);
}

#include "ShLpImpl.hpp"
