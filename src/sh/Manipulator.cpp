...
template<>
SH_DLLEXPORT
int OffsetRange<int>::absIndex( int index, int offset, 
  const ProgramNode::VarIt &V, size_t vsize ) const {
  if( offset == _FIRST ) return 0;
  else if( offset == _SECOND ) return 1;
  else if( offset == _LAST ) return vsize - 1;
  if( index < 0 ) index += vsize;
  if( index < 0 || index >= static_cast<int>(vsize) ) return OFFSET_RANGE_BAD_INDEX; 

  index += offset;
  if( index < 0 || index >= static_cast<int>(vsize) ) return OFFSET_RANGE_BAD_OFFSET; 
  return index;
}
...
int OffsetRange<std::string>::absIndex( std::string index, int offset, 
  ...
  if( result == OFFSET_RANGE_BAD_INDEX ) {
    ...
    ProgramNode::print(os, V, vsize) << std::endl;
    error( AlgebraException( os.str()));
  }

  result += offset;
  if( result < 0 || result >= static_cast<int>(vsize)) result = OFFSET_RANGE_BAD_OFFSET;
  return result; 
}
