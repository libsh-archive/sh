 ...
 */
template<typename T>
void lpPivot(size_t M, size_t N, std::valarray<size_t>& nonBasic, std::valarray<size_t>& basic, 
 ...
 */
template<typename T>
void lpSimplexCore(size_t M, size_t N, std::valarray<size_t>& nonBasic, std::valarray<size_t>& basic, 
    std::valarray<std::valarray<T> >& A, std::valarray<T> &b, 
    std::valarray<T> &c, T& v)
{
  ...
  for(;;) {
    ...
    if(l == -1) throw UnboundedLpException();
    lpPivot(M, N, nonBasic, basic, A, b, c, v, l, e); 
...
template<typename T>
void lpSimplexInit(size_t M, size_t N, std::valarray<size_t>& nonBasic, std::valarray<size_t>& basic, 
    std::valarray<std::valarray<T> >& A, std::valarray<T> &b, 
    std::valarray<T> &c, T& v)
{
  size_t i, j, k;
...
template<typename T>
std::valarray<T> lpSimplex(size_t M, size_t N, const std::valarray<T>& A, 
    const std::valarray<T>& b, const std::valarray<T>& c)
{
  ...
  lpSimplexInit(M, N, nonBasic, basic, Ap, bp, cp, v); 
  lpSimplexCore(M, N, nonBasic, basic, Ap, bp, cp, v);
...
template<typename T>
std::valarray<T> lpSimplexRelaxed(size_t M, size_t N, const std::valarray<T>& A, 
    const std::valarray<T>& b, const std::valarray<T>& c, 
    const std::valarray<bool>& bounded)
{
  ...
  std::valarray<T> xp = lpSimplex(M, Np, Ap, b, cp);
