...
void RangeOp<OP, T1, T2>::operator()( 
    Variant* dest, const Variant* a, const Variant* b, const Variant* c) const
{

  SH_DEBUG_ASSERT(dest && a);

  DataVariant<T1, HOST>* destVec = variant_cast<T1, HOST>(dest);

  const DataVariant<T2, HOST>* aVec = variant_cast<T2, HOST>(a);
  const DataVariant<T2, HOST>* bVec = b ? variant_cast<T2, HOST>(b) : 0;

  ConcreteRangeOp<OP, T1, T2>::doop(destVec, aVec, bVec);
}
...
void __initRangeOps() {
  Eval* eval = Eval::instance();
  const ValueType V = StorageTypeInfo<T>::value_type;
  const ValueType RangeV = StorageTypeInfo<RangeT>::value_type; 

  eval->addOp(OP_LO, new RangeOp<OP_LO, T, RangeT>(), 
      V, RangeV);
  eval->addOp(OP_HI, new RangeOp<OP_HI, T, RangeT>(), 
      V, RangeV);

  eval->addOp(OP_WIDTH, new RangeOp<OP_WIDTH, T, RangeT>(), 
      V, RangeV);
  eval->addOp(OP_RADIUS, new RangeOp<OP_RADIUS, T, RangeT>(), 
      V, RangeV);
  eval->addOp(OP_CENTER, new RangeOp<OP_CENTER, T, RangeT>(), 
      V, RangeV);


  eval->addOp(OP_UNION, new RangeOp<OP_UNION, RangeT, RangeT>(), 
      RangeV, RangeV, RangeV);

  eval->addOp(OP_ISCT, new RangeOp<OP_ISCT, RangeT, RangeT>(), 
      RangeV, RangeV, RangeV);

  eval->addOp(OP_CONTAINS, new RangeOp<OP_CONTAINS, RangeT, RangeT>(), 
      RangeV, RangeV, RangeV);
}

template<typename T, typename IntervalT>
void _initIntervalOps() {
  __initRangeOps<T, IntervalT>();

  Eval* eval = Eval::instance();
  const ValueType V = StorageTypeInfo<T>::value_type;
  const ValueType IntervalV = StorageTypeInfo<IntervalT>::value_type; 

  eval->addOp(OP_IVAL, new RangeOp<OP_IVAL, IntervalT, T>(), 
      IntervalV, V, V);
}

template<typename T, typename AffineT>
void _initAffineOps() {
  __initRangeOps<T, AffineT>();
}
