...
public:
  ...
  /// Prints out statistics about the program
  /// (variable counts for in/out/temp, number and distribution of instructions, code size 
  std::string describe_counts();
  ...
  /// Called whenever decls/vars need to be collected as a precondition for some 
  /// processing on the graph.  This is useful when the graph may have been
  /// modified since the last optimization pass. 
  void collectAll();
  ...
  typedef VarList::const_iterator VarIt;
...
private:
  ...
  /* used in cleaning decls */
  VarSet usedTempDecls; ///< variables declared and used in this program (only available after collectDecls);

  void collect_node_decls(CtrlGraphNode* node);
  void clean_node_decls(CtrlGraphNode* node);
  void collect_var_decl(const VariableNodePtr& node);
  ...
  typedef std::map<Operation, int> OpStats;
  void collect_stats(OpStats& stats);
  void collect_stats_helper(const CtrlGraphNode* node, OpStats& stats);
