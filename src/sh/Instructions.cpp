...
INST_UNARY_OP(HI);
INST_UNARY_OP(WIDTH);
INST_UNARY_OP(RADIUS);
INST_UNARY_OP(CENTER);

INST_BINARY_OP(IVAL, true, true);
INST_BINARY_OP(UNION, true, true);
INST_BINARY_OP(ISCT, true, true);
INST_BINARY_OP(CONTAINS, true, true);

void shERRFROM(Variable& dest, const Variable& a, const Variable& b)
{
  sizes_match(dest, a);
  if(immediate()) {
    error(ScopeException("Cannot compute affine err_from right now in immediate mode")); 
  }
  Statement stmt(dest, a, OP_ERRFROM, b);
  addStatement(stmt);
}

void shLASTERR(Variable& dest, const Variable& a, const Variable& b)
{
  sizes_match(dest, a);
  SH_DEBUG_ASSERT(b.size() == 1);
  if(immediate()) {
    error(ScopeException("Cannot compute affine lasterr right now in immediate mode")); 
  }
  Statement stmt(dest, a, OP_LASTERR, b);
  addStatement(stmt);
}
...
void shDBG(const Variable& a)
{
  if(immediate()) {
      error(ScopeException("Cannot dbg in immediate mode"));
  }
  SH_DEBUG_ASSERT(!immediate());
  Statement stmt(a, OP_DBG, a);
  addStatement(stmt);
}
