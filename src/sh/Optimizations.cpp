...
struct Straightener {
  ...
  void operator()(CtrlGraphNode* node)
  {
     ...
     * This is important, since some of the other optimizations only work within a single block,
     * so we want things straight early. 
     * This will eventually end, since each step eats a CFG node */
    for(;;) { 
...
struct EmptyBlockRemover {
  ...
  void operator()(CtrlGraphNode* node)
  {
    ...
    if(node->hasDecls()) {
      node->follower()->insert_decls(node->decl_begin(), node->decl_end());
    }
...
struct CommonStatementRemover {
  typedef BasicBlock::StmtList::iterator BlkIt;
  typedef std::list<BlkIt> StmtList; 
  StmtList m_can_merge; 
  bool& changed;

  CommonStatementRemover(bool& changed)
    : changed(changed)
  {
  }

  bool matches(const Statement& s, const Statement& t) {
      if(s.op != t.op || s.dest != t.dest) return false;
      if(s.src.size() != t.src.size()) return false;
      for(size_t i = 0; i < s.src.size(); ++i) {
        if(s.src[i] != t.src[i]) return false;
      }
      return true;
  }
  void operator()(CtrlGraphNode* node) {
    ...
    BasicBlockPtr block = node->block;
    if (!block) return;
    for (BlkIt I = block->begin(); I != block->end();) {
      if(I->dest.null()) continue;

      bool killI = false;
      for (StmtList::iterator J = m_can_merge.begin(); J != m_can_merge.end();) {
        if (inRHS(I->dest.node(), **J) ||
           I->dest.node() == (*J)->dest.node()) {
          J = m_can_merge.erase(J);
        } else if (matches(*I, **J)) {
          killI = true;
          break;
        } else ++J; 
...
void remove_common_statements(Program& p, bool& changed) 
{
  CommonStatementRemover csr(changed);
  p.node()->ctrlGraph->dfs(csr);
}
