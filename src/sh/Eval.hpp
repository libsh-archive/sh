...
/** A RangeOp is one of the special ops that only operate 
 * on range types (some also take one non-range arg)
 */
template<Operation S, typename T1, typename T2>
struct RangeOp: public EvalOp {
  void operator()(Variant* dest, const Variant* a, 
      const Variant* b, const Variant* c) const; 
};

template<Operation S, typename T1, typename T2>
struct ConcreteRangeOp{
  static void doop(DataVariant<T1, HOST> *dest, 
      const DataVariant<T2, HOST> *a, const DataVariant<T2, HOST> *b = 0);
};
...
// initializes the interval ops for a type T and Interval<T>
template<typename T, typename IntervalT>
void _initIntervalOps();

// initializes the affine ops for a type T and Affine<T>
...
#include "ConcreteRangeOpImpl.hpp"
//#include "IntervalEvalImpl.hpp"
