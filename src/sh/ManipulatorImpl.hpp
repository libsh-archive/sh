...
template<typename T>
int OffsetRange<T>::absStartIndex( const ProgramNode::VarIt& V, size_t vsize ) const {
  return absIndex( start, startOffset, V, vsize );
}

template<typename T>
int OffsetRange<T>::absEndIndex( const ProgramNode::VarIt& V, size_t vsize ) const {
  return absIndex( end, endOffset, V, vsize );
}
...
Program operator<<(const Program &p, const Manipulator<T> &m) {
  ...
  Program permuter = SH_BEGIN_PROGRAM() {
    ...
    ProgramNode::VarIt freeStart = p.begin_free_inputs();
    size_t freeSize = p.free_input_count();
    ProgramNode::VarIt inIt = freeStart; 
    ...
    for(typename RangeVec::const_iterator irvIt = mranges.begin();
      ...
      int start = irvIt->absStartIndex( freeStart, freeSize ); 
      int end = irvIt->absEndIndex( freeStart, freeSize ); 
