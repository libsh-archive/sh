...
public:
  ...
  /* @todo range - calling this "uniform" may be a bit confusing, if we allow nested program calls 
   * (range arithmetic branch supports these) */
  Record uniform_inputs;


  std::string describe_binding_spec() const;
  ProgramNode::VarIt begin_free_inputs() const {
    ProgramNode::VarIt result = m_node->begin_inputs();
    std::advance(result, binding_spec.size());
    return result;
  }

  size_t free_input_count() const {
    return m_node->inputs.size() - binding_spec.size();
  }

  /* Sets the bindings of this to those of other */
  void clone_bindings_from(const Program& other);

  /* Appends the binding spec from other to the end of this */ 
  void append_bindings_from(const Program& other);
