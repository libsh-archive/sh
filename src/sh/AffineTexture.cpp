...
AffineTexture::AffineTexture(TextureNodePtr other) 
  : m_other(other),
    m_width(other->width() * AATEX_EXPAND)
{
  ...
  m_node = new TextureNodePtr[m_dims + 2];
  for(size_t i = 0; i < m_dims + 2; ++i) {
    m_node[i] = new TextureNode(other->dims(), other->size(), SH_FLOAT, 
...
void AffineTexture::memory_update() {
  size_t i, j;
  ...
  for(; cur_width <= widths[FIRST]; cur_width *= 2, cur_length /= 2) {
    ...
    for(i = 0; i < tupleSize; ++i) {
      ...
      for(;; ++R, ++O) { // assuming this happens at least once...else bad
        ...
        float_array x( lpSimplexRelaxed(M, OUTPUTS, 
...
VariableNodePtr AffineTexture::makeTemp(const Variable &v, const std::string& name, int size) 
{
  if(size == 0) size = v.size();
  VariableNodePtr result = v.node()->clone(SH_TEMP, size, SH_VALUETYPE_END, SH_SEMANTICTYPE_END, true, false); 
  result->name(name);
  return result;
}

Record AffineTexture::rect_lookup(const Variable &lo, const Variable &hi) const 
{
  Record result;
  SH_BEGIN_SECTION("aatex_rect_lookup") {
    ...
    Variable diff(makeTemp(lo, "diff")); // difference threshold between cells to force a level increase
    shADD(diff, HALF.repeat(size), gt0);

    Variable cell_lo(makeTemp(lo, "cell_lo"));
    Variable cell_hi(makeTemp(hi, "cell_hi"));
    ...
    Variable inc_level(makeTemp(lo, "inc_level")); // should be bool - whether to increase level
    shADD(inc_level, cell_hi, -cell_lo);
    shSGT(inc_level, inc_level, diff);

    shADD(level, level, inc_level);

    Variable max_level(makeTemp(lo, "max_level", 1));
    if(level.size() == 1) {
    ...
    Variable offset(makeTemp(lo, "offset", 1));
    Variable texscale(makeTemp(lo, "texscale", 1));
    ...
    shMUL(offset, offset, ConstAttrib1f(m_width));
    ...
    Variable texcoord0 = texcoord(0);
    shADD(texcoord0, texcoord0, offset);

    for(size_t i = 0; i < m_dims + 2; ++i) {
      Variable resulti(m_node[i]->clone(SH_TEMP, 0, SH_VALUETYPE_END, SH_SEMANTICTYPE_END, true, false));
      Variable texVar(m_node[i]);

      Statement stmt(resulti, texVar, OP_TEXI, texcoord);
...
Record AffineTexture::lookup(const Variable& lo, const Variable& hi) const {
  ...
  Record result = rect_lookup(scaled_lo, scaled_hi);

  // scale slopes 
  Record::iterator I = result.begin();
...
TextureNodePtr AffineTexture::node(size_t i) {
  return m_node[i];
}

AffineTexture::TexAaTexMap AffineTexture::tatmap;

AffineTexturePtr AffineTexture::convert(TextureNodePtr other) 
{
  TexAaTexMap::iterator I = tatmap.find(other);
  if(I != tatmap.end()) {
    return I->second;
  }
  return (tatmap[other] = new AffineTexture(other));
}
