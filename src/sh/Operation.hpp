...
enum Operation {
  ...
  // Special
  OP_DBG, ///< @todo range stupid hack for debugging
  ...
  OP_PHI,    ///< Used in certain range arithmetic transforms as an SSA phi function 
                ///  This should never show up in code passed to the backend.
                ///  Right now it is also not safe to use this in the optimizer.
  OP_ESCJOIN, ///< Acts as an assignment in hierarchical AA where error symbols
                 /// for variables that escape a hierarchical region are merged together
  OP_ESCSAV,  ///< Stores the max contribution of pruned hierarchical symbols  
  ...
  OP_LO,  ///< Extracts a lower bound from a range arithmetic type
  OP_HI,  ///< Extracts an upper bound from a range arithmetic type
  OP_WIDTH, ///< Returns the width of a range type 
  OP_RADIUS, ///< Returns the radius = width/2 of a range type 
  OP_CENTER, ///< Returns the center of a range type 

  OP_IVAL, ///< Make an interval out of the given lower bound (src[0]) and upper bound (src[1])
  OP_UNION, ///< Union two ranges 
  OP_ISCT, ///< Intersect two ranges 
  OP_CONTAINS, ///< Intersect two ranges 

  OP_ERRFROM, ///< Binary operation, returns affine error in src[0] due to src[1]
               /// Result is an affine term centered at 0 containing the
               /// appropriate error terms. 
               
  OP_LASTERR, ///< Like err_from, but returns just the magnitudes from the last error term in src[1],
