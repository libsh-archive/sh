...
void TypeInfo::addCasts()
{
  ...
  addPromotion<Affine<double>, Affine<float> >();

  // @todo range do we really want to allow conversions from affine -> interval?
  addPromotion<Affine<double>, Interval<double> >(); 

  addPromotion<Affine<float>, Interval<float> >();

  
  addPromotion<Interval<double>, Interval<float> >();
  addPromotionOnly<Interval<double>, double>();

  addPromotionOnly<Interval<float>, float>();
 ...
 // these are the memory->HOST, HOST->memory casts
  addMemoryCast<Affine<double> >();
  addMemoryCast<Affine<float> >();

  addMemoryCast<Interval<double> >();
  addMemoryCast<Interval<float> >();
