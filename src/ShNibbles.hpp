...
ShProgram access(const ShBaseTexture1D<T, traits> &tex, const std::string & name = "result");

/** \brief 2D texture lookup nibble 
 * Inputs: IN(0) ShTexCoord2f tc
 * Outputs: OUT(0) name = tex(tc) */
template<typename T, unsigned int traits> 
ShProgram access(const ShBaseTexture2D<T, traits> &tex, const std::string & name = "result");

/** \brief Rect texture lookup nibble 
 * Inputs: IN(0) ShTexCoord2f tc
 * Outputs: OUT(0) name = tex(tc) */
template<typename T, unsigned int traits> 
ShProgram access(const ShBaseTextureRect<T, traits> &tex, const std::string & name = "result");

/** \brief 3D texture lookup nibble 
 * Inputs: IN(0) ShTexCoord3f tc
 * Outputs: OUT(0) name = tex(tc) */
template<typename T, unsigned int traits> 
ShProgram access(const ShBaseTexture3D<T, traits> &tex, const std::string & name = "result");

/** \brief Cube texture lookup nibble 
 * Inputs: IN(0) ShTexCoord3f tc
 * Outputs: OUT(0) name = tex(tc) */
template<typename T, unsigned int traits> 
ShProgram access(const ShBaseTextureCube<T, traits> &tex, const std::string & name = "result");
...
ShProgram transform(const ShMatrix<Rows, Cols, Kind, T> &m, const std::string & name = "result");
