...
{
  if (i0 < 0 || i0 >= srcSize) throw ShSwizzleException(i0, srcSize);
...
ShSwizzle::ShSwizzle(int srcSize, int i0, int i1)
{
  if (i0 < 0 || i0 >= srcSize) throw ShSwizzleException(i0, srcSize);
  m_indices.push_back(i0);
  if (i1 < 0 || i1 >= srcSize) throw ShSwizzleException(i1, srcSize);
...
ShSwizzle::ShSwizzle(int srcSize, int i0, int i1, int i2)
{
  ...
  if (i1 < 0 || i1 >= srcSize) throw ShSwizzleException(i1, srcSize);
  m_indices.push_back(i1);
  if (i2 < 0 || i2 >= srcSize) throw ShSwizzleException(i2, srcSize);
...
ShSwizzle::ShSwizzle(int srcSize, int i0, int i1, int i2, int i3)
{
  ...
  if (i2 < 0 || i2 >= srcSize) throw ShSwizzleException(i2, srcSize);
  m_indices.push_back(i2);
  if (i3 < 0 || i3 >= srcSize) throw ShSwizzleException(i3, srcSize);
...
ShSwizzle::ShSwizzle(int srcSize, int size, int* indices)
{
  for (int i = 0; i < size; i++) {
    if (indices[i] < 0 || indices[i] >= srcSize) throw ShSwizzleException(indices[i], srcSize);
...
ShSwizzle::ShSwizzle& ShSwizzle::operator*=(const ShSwizzle& other)
{
  ...
  for (int i = 0; i < other.size(); i++) {
    if (other[i] < 0 || other[i] >= size()) {
      throw ShSwizzleException(other[i], size());
    }
...
{
  if (index >= size() || index < 0) throw ShSwizzleException(index, size());
  return m_indices[index];
}

ShSwizzleException::ShSwizzleException(int index, int size)
  : ShException("")
{
  std::ostringstream out;
  out << "Swizzle error: " << index << " out of range [0, " << size << ")" << std::ends;
