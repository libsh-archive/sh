...
template<int N, ShBindingType B1, ShValueType V, bool S1>
  ShNormal<N, SH_TEMP, V, false>
  abs(const ShNormal<N, B1, V, S1>& var)
  {
    ShGeneric<N, V> t = abs(static_cast< ShGeneric<N, V> >(var));
    ShNormal<N, SH_TEMP, V, false> vec(t.node(), t.swizzle(), t.neg());
    return vec;
  }

template<int N, ShBindingType B1, ShValueType V, bool S1>
  ShNormal<N, SH_TEMP, V, false>
  normalize(const ShNormal<N, B1, V, S1>& var)
  {
...
ShGeneric<1, V> operator|(const ShNormal<N, B1, V, S1>& a,
                            const ShNormal<N, B2, V, S2>& b)
{
  return dot(a, b);
}
...
ShNormal<3, SH_TEMP, V, false> operator|(const ShMatrix<4, 4, B1, V>& m,
                                             const ShNormal<3, B2, V, S1>& v)
{
  ShNormal<3, SH_TEMP, V, false> t;
  for (int i = 0; i < 3; i++) {
    t(i) = dot(m[i](0,1,2), v);
  }
  return t;
...
template<ShBindingType B1, ShBindingType B2, ShValueType V, bool S1>
ShNormal<2, SH_TEMP, V, false> operator|(const ShMatrix<3, 3, B1, V>& m,
                                             const ShNormal<2, B2, V, S1>& v)
{
  ShNormal<2, SH_TEMP, V, false> t;
  for (int i = 0; i < 2; i++) {
    t(i) = dot(m[i](0,1), v);
  }
  return t;
...
template<ShBindingType B1, ShBindingType B2, ShValueType V, bool S1>
ShNormal<1, SH_TEMP, V, false> operator|(const ShMatrix<2, 2, B1, V>& m,
                                         const ShNormal<1, B2, V, S1>& v)
{
  ShNormal<1, SH_TEMP, V, false> t;
  for (int i = 0; i < 1; i++) {
    t(i) = dot(m[i](0), v);
  }
  return t;
