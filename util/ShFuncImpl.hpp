...
ShGeneric<1, V> lInfDistance(const ShGeneric<N, V>& a, const ShGeneric<N, V>& b) {
  ShGeneric<N, V> diff = abs(a - b);
  ShGeneric<1, V> result = max(diff(0), diff(1));
...
void groupEvenOddSort(ShAttrib<N, Binding, V> v[]) {
  ...
  ShAttrib<NU, SH_TEMP, V> eu, ou, ccu; 
  ShAttrib<ND, SH_TEMP, V> ed, od, ccd; 
  ...
  for(i = 0; i < S; ++i) {
    ShAttrib<NE, SH_TEMP, V> evens = v[i].template swiz<NE>(eswiz);
...
void groupBitonicSort(ShAttrib<N, Binding, V> v[]) {
  ...
  bool doFlip;
  int swiz[2][C];
  ShAttrib<C, SH_TEMP, V> temp[2], condition; 
